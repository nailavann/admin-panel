...
class ImageHelper
{
     ...
     * @throws Throwable
     */
    protected function createFile($file, $path): string
    {
        throw_unless($file, Exception::class, 'File is empty!');

        $name = Str::random(25) . "." . $this->getExtension();

        throw_unless($file->storeAs($path, $name, 'public'), Exception::class, 'Could not saved!');
    ...
    {
        $decodedImage = ImageManagerStatic::make($this->getFile())->encode($this->getExtension(), 80);
        $path = $this->createPath($this->getDirectory(), $this->getExtension());
        return $this->createImage($decodedImage, $path);
    }
    ...
    {
        $path = $this->createPath($this->getDirectory(), $this->getExtension(), false);
     ...
     * @throws Throwable
     */
    public function getFileStorage($path): string
    {
        return $this->getFileSystem()->temporaryUrl($path, now()->addMinutes(5));
    }
